...
#==============================================================================
# Azure App Registration Setup Script (Multitenant)
#==============================================================================
...
#              Configured for MULTITENANT support (Any Microsoft Entra ID tenant)
...
create_app_registration() {
    ...
    if [[ -z "$APP_ID" ]]; then
        log_info "Creating new multitenant app registration: $APP_NAME"
        log_info "  Sign-in audience: Accounts in any organizational directory (Any Microsoft Entra ID tenant - Multitenant)"
        ...
        log_success "Multitenant app registration created successfully"
        log_info "  App ID: $APP_ID"
        log_info "  Sign-in audience: AzureADMultipleOrgs (Multitenant)"
    else
        log_warn "App registration '$APP_NAME' already exists"
        log_info "  App ID: $APP_ID"
        
        # Check current sign-in audience
        local current_audience=$(az ad app show --id "$APP_ID" --query signInAudience -o tsv 2>/dev/null || echo "unknown")
        log_info "  Current sign-in audience: $current_audience"
        
        # Update to multitenant if not already set
        if [[ "$current_audience" != "AzureADMultipleOrgs" ]]; then
            log_info "Updating app registration to multitenant..."
            az ad app update --id "$APP_ID" --sign-in-audience AzureADMultipleOrgs &>/dev/null || {
                log_warn "Failed to update sign-in audience, but continuing..."
            }
            log_success "App registration updated to multitenant"
        fi
...
display_results() {
    ...
    # Get the current sign-in audience for display
    local sign_in_audience=$(az ad app show --id "$APP_ID" --query signInAudience -o tsv 2>/dev/null || echo "Unknown")
    local audience_description=""
    
    case "$sign_in_audience" in
        "AzureADMultipleOrgs")
            audience_description="Multitenant (Any Microsoft Entra ID tenant)"
            ;;
        "AzureADMyOrg")
            audience_description="Single tenant (This directory only)"
            ;;
        "AzureADandPersonalMicrosoftAccount")
            audience_description="Multitenant + Personal Microsoft accounts"
            ;;
        "PersonalMicrosoftAccount")
            audience_description="Personal Microsoft accounts only"
            ;;
        *)
            audience_description="Unknown ($sign_in_audience)"
            ;;
    esac
...
Sign-in Audience: $audience_description
...
main() {
    log_info "Starting Azure App Registration setup (Multitenant)..."
